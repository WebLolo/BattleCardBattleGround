...
export default function Game1v1(){
    ...
    const jouerCarteDepuisDeck = (card) => {
        if (actualPlayer === 1) {
          ...
          const nouveauDeck = deck.filter((c) => c.id !== card.id);
        ...
        if (actualPlayer === 2) {
          ...
          const nouveauDeck = deckPlayer2.filter((c) => c.id !== card.id);
    ...
    const handleDragEnd = (event) => {
        if(actualPlayer === 1){
            ...
            if (sourceType === "shop" && targetType === "footer" && gold >= 3) {
                ...
                const nouveauDeck = jouerCarteDepuisDeck(draggedCard);
                if (!nouveauDeck) return;
            
                const cloneSpe = piocherCarteSpe(
                    sourceType,
                    targetType,
                    draggedCard,
                    nouveauDeck,
                ...
                setDeck(cloneSpe ? [...nouveauDeck, cloneSpe] : nouveauDeck); // <- Mise à jour propre
            
                // 💡 Petite pause pour laisser React faire son rendu avant la fusion
                setTimeout(() => {
                    if (cloneSpe) {
                        fusionnerCartesIdentiques({
                            carteBase: cloneSpe,
                            deck: [...nouveauDeck, cloneSpe],
                            board: [...boardPlayer],
                            setDeck,
                            setBoard: setBoardPlayer
                        });
                ...
                }, 50); // ⏳ assez court pour éviter la duplication
            }
        ...
        if(actualPlayer === 2){
            ...
            piocherCarte( sourceType, targetType, draggedCard, deckPlayer2, setDeckPlayer2, boardPlayer2, setBoardPlayer2 )
            ...
            const cloneSpe = piocherCarteSpe(
              ...
              if (cloneSpe) {
                fusionnerCartesIdentiques({
                  carteBase: cloneSpe,
                  deck: [...deckPlayer2, cloneSpe], // on simule le deck mis à jour
                  board: boardPlayer2,
                  setDeck: setDeckPlayer2,
                  setBoard: setBoardPlayer2
                });
