...
;
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        if ((phase === "shopPlayer1" || phase === "shopPlayer2") && audioRef.current.paused) {
          // Ne joue que si elle n'est pas déjà en cours
          audioRef.current.play().catch((e) => {
            console.warn("Impossible de jouer la musique :", e);
          });
        }
      
        if (phase === "combat" && !audioRef.current.paused) {
          audioRef.current.pause();
          audioRef.current.currentTime = 0;
        }
      
        return () => {
          if (phase === "combat" && audioRef.current) {
            audioRef.current.pause();
            audioRef.current.currentTime = 0;
          }
        };
