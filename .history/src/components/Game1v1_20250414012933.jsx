...
useEffect(() => {
  ...
  const fadeIn = () => {
    ...
    fadeInterval.current = setInterval(() => {
      if (audioRef.current.volume < 0.4) {
        audioRef.current.volume = Math.min(audioRef.current.volume + 0.02, 0.4);
      } else {
        clearInterval(fadeInterval.current);
      }
    }, 100);
  ...
  if (phase === "shopPlayer1" || phase === "shopPlayer2") {
    if (audioRef.current.paused) {
      audioRef.current.play().then(fadeIn).catch((e) => console.warn("🎵 Lecture fail:", e));
    } else {
      fadeIn();
    }
