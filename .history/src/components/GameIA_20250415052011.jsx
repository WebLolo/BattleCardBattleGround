...
import { TourIa } from "@/utils/IaUtils";
...
export default function GameIA(){
        ...
        const [fusionAnimation, setFusionAnimation] = useState(null);
        const refreshRef = useRef(null);
        const shopSnapshotRef = useRef([])
        ...
        async function waitForShopUpdate(previousShop) {
            return new Promise((resolve) => {
              const check = () => {
                if (shopSnapshotRef.current !== previousShop) {
                  resolve();
                } else {
                  setTimeout(check, 100); // vérifie toutes les 100ms
                }
              };
              check();
            });
        ...
        useEffect(() => {
          ...
          }, [lvlTavernePlayer2, goldPlayer2, setShopCards, setGoldPlayer2]);
          useEffect(() => {
            shopSnapshotRef.current = shopCards;
          }, [shopCards]);

          useEffect(() => {
            if (phase === "shopPlayer2") {
                console.log("🛒 Cartes du shop au lancement IA :", shopCards);
              const launchIa = async () => {
                // Attendre que le shop soit bien généré
                while (shopCards.length === 0) {
                  await sleep(100); // petite pause, on attend le render du ShopBoard
                }
          
                // IA démarre une fois le shop en place
                TourIa({
                  goldPlayer2,
                  setGoldPlayer2,
                  shopCards,
        ...
        return (
                ...
                <DndContext onDragEnd={handleDragEnd}>
                            ...
                            phase === "shopPlayer1" ? (
                                ...
                                <DropZone id="header" className="drop-header">
                                    <ShopHUD
                                        ...
                                        onRefresh={(...args) => {
                                            refreshRef.current?.(...args); // on garde l’appel réel
                                            actualiserBoutique({ lvlTavernePlayer2, setShopCards, goldPlayer2, setGoldPlayer2, actualPlayer });   // on appelle vraiment la fonction
                                          }}
                                        onLvlUp={() =>
                    ...
                    {/* <FusionAnimation fusionAnimation={fusionAnimation} onFinish={() => setFusionAnimation(null)} /> */}
                </DndContext>
            )
}