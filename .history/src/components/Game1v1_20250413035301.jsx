...
import Card from "@/components/Card"; // Le même que tu utilises déjà
import { closestCenter, MeasuringStrategy } from "@dnd-kit/core";
...
const handleDragStart = (event) => {
    const { active } = event;
    const [origin, cardId] = active.id.split("-");
    const id = parseInt(cardId);
  
    let foundCard = null;
  
    if (origin === "board") {
      foundCard = boardPlayer.find((c) => c.id === id) || boardPlayer2.find((c) => c.id === id);
    } else if (origin === "deck") {
      foundCard = deck.find((c) => c.id === id) || deckPlayer2.find((c) => c.id === id);
    } else if (origin === "shop") {
      foundCard = shopCards.find((c) => c.id === id);
    }
  
    if (foundCard) setDraggedCard(foundCard);
  };
  ...
  if (actualPlayer === 1) {
    ...
    // 👉 Si la carte vient du board et va vers une autre position du board
    ...
    // 🧩 Changement de position sur le board
    if (sourceType === "board" && targetType.startsWith("board-")) {
      const targetIndex = parseInt(targetType.split("-")[1]);
      const currentIndex = boardPlayer.findIndex((c) => c.id === draggedCard.id);
      if (currentIndex !== -1 && targetIndex !== -1 && targetIndex !== currentIndex) {
        const newBoard = [...boardPlayer];
        const [removed] = newBoard.splice(currentIndex, 1);
        newBoard.splice(targetIndex, 0, removed);
        setBoardPlayer(newBoard);
        return;
      }
    }
    ...
    return (
            <DndContext onDragStart={handleDragStart} // 👈 ici
  ...
  {draggedCard ? (
    <img
      src={draggedCard.imgMinia}
      alt={draggedCard.nom}
      style={{ width: "100px", height: "auto" }} // adapte à ton style
    />
  ) : null}
</DragOverlay>
            </DndContext>
