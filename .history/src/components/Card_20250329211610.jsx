...
export default function Card({
  ...
  useEffect(() => {
    ...
    controls.start({
      x: deltaX,
      y: deltaY,
      transition: { duration: 0.3, ease: [0.2, 0, 0.6, 1] }, // ⚡ Accéléré
    }).then(() => {
      controls.start({
        x: 0,
        y: 0,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className={`hudIntAtk ${card.atk > card.baseAtk ? "buffed" : ""}`}>{card.atk}</p>
        <p className={`hudIntPv  ${phase === "combat" && card.hp < card.baseHp ? "pv-damaged" : ""} ${card.hp > card.baseHp ? "buffed" : ""} `}>{card.hp}</p>
