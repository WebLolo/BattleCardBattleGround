...
export default function Card({
  ...
  carteAttaquantId,
  carteDefenseurId,
  isDraggingOverlay = false, // ✅ renommé pour éviter conflit
}) {
  ...
  const style = {
    transform: transform
      ...
      : undefined,
    cursor: "grab",
    zIndex: isDragging || isAttaquant ? 999 : 1,
    position: "relative",
    opacity: isDragging ? 0 : 1, // 👈 MASQUE la carte pendant le drag
  };
      ...
      >
        <motion.div
          ...
          className={`cardfight player content-card 
            ${shouldAnimate && !isDraggingOverlay ? "animate__animated animate__backInLeft" : ""}
        ...
        >
          <AnimatePresence mode="wait">
            <motion.img
              key={card.provocationUse ? "provoc" : "normal"}
              ...
              className="cardimg"
              src={
                card.provocationUse && card.imgMiniaProvoc
                  ? card.imgMiniaProvoc
                  : card.imgMinia
              }
              alt={card.nom}
