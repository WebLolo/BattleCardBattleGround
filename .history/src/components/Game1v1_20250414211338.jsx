    ...
    const handleDragEnd = (event) => {
        if(actualPlayer === 1){
            ...
            if (sourceType === "shop" && targetType === "footer" && gold >= 3) {
                ...
                if (cloneSpe) {
                  // 🕒 Attendre 50ms pour laisser Drush apparaître dans le board
                  setTimeout(() => {
                    fusionnerCartesIdentiques({
                      carteBase: cloneSpe,
                      deck: [...deck, cloneSpe], // deck est déjà mis à jour dans piocherCarteSpe
                      board: [...boardPlayer, draggedCard], // <- on ajoute Drush à la main si nécessaire
                      setDeck,
                      setBoard: setBoardPlayer
                    });
                  }, 50);
