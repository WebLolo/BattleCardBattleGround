...
useEffect(() => {
    ...
    const fadeOut = (audio, callback, speed = 0.02) => {
      clearInterval(fadeInterval.current);
      fadeInterval.current = setInterval(() => {
        if (audio.volume > 0) {
          audio.volume = Math.max(audio.volume - speed, 0);
        } else {
          audio.pause();
          clearInterval(fadeInterval.current);
    ...
    const fadeIn = (audio, maxVolume = 0.4, speed = 0.02) => {
      clearInterval(fadeInterval.current);
      audio.volume = 0;
      audio.play();
      fadeInterval.current = setInterval(() => {
        if (audio.volume < maxVolume) {
          audio.volume = Math.min(audio.volume + speed, maxVolume);
        } else {
          clearInterval(fadeInterval.current);
        }
      }, 50); // ⏱️ plus rapide
    ...
    return (
            <DndContext onDragEnd={handleDragEnd}>
                ...
                <MusicControl audioRefs={[shopMusicRef, combatMusicRef]} />
            </DndContext>
        )
}