...
export default function Game1v1(){
    ...
    // Pv Player1
    const [playerPv, setplayerPv] = useState(300);
    ...
    // Pv Player 2
    const [player2Pv, setplayer2Pv] = useState(300);
...
;
    ...
    useEffect(() => {
        if (projectileAnim) {
          console.log("🎯 ProjectileAnim reçu dans Game1v1", projectileAnim);
        }
      }, [projectileAnim]);
    useEffect(() => {
    ...
    return (
            <DndContext onDragEnd={handleDragEnd}>
                ...
                {projectileAnim && (
                    console.log("🎯 Rendu du ProjectileImage"),
                    <ProjectileImage
                        key={`${projectileAnim.startX}-${projectileAnim.endX}-${Date.now()}`} // ← key unique à chaque projectile
                        startX={projectileAnim.startX}
                        startY={projectileAnim.startY}
                        endX={projectileAnim.endX}
                        endY={projectileAnim.endY}
                        onEnd={() => setProjectileAnim(null)}
