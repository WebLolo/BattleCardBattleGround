...
;
    ...
    const handleDragEnd = (event) => {
        if(actualPlayer === 1){
            ...
            if (sourceType === "deck" && targetType === "board-drop" && draggedCard.criDeGuerreUniqueCroc && boardPlayer.length <7 ) {
                console.log(`🎯 Cri de guerre ciblé sur UNE seule carte pour ${draggedCard.nom}`);
                const crocNoirSousFamille = board.filter(c => c.famille === "Croc-Noir" && c.id !== draggedCard.id && c.sousFamille === draggedCard.sousFamille);
                let cible = crocNoirSousFamille[Math.floor(Math.random() * crocNoirSousFamille.length)];
                draggedCard.criDeGuerreUniqueCroc(cible, draggedCard, boardPlayer); // Effet sur tout le board
            }
