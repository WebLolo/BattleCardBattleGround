  ...
  if (tourIa === 3) {
    ...
    while (gold >= 3) {
      ...
      if (!carte) {
        ...
        const previous = shopSnapshotRef.current;
        refreshRef.current?.(); // Simule clic IA
        await waitForShopUpdate(previous); // ⏳ Attendre vraiment que ça change
        await sleep(300); // (optionnel) délai de sécurité
        continue; // Réessayer avec le nouveau shop
      }
