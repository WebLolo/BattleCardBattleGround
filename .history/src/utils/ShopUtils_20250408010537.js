...
export const clonerCarte = ({ carte, camp }) => {
  return {
      ...
      bivalenceMerEffect: carte.bivalenceMerEffect !== undefined ? carte.bivalenceMerEffect : false,
      bivalenceTerreEffect: carte.bivalenceTerreEffect !== undefined ? carte.bivalenceTerreEffect : false,
  };
}
...
// Bivalence application
if (sourceType === "deck" && targetType === "board-drop" && draggedCard.bivalence && boardPlayer.length <7) {
  console.log(`📢 Bivalence activé par ${draggedCard.nom}`);
  draggedCard.bivalence(boardPlayer)

}
if (sourceType === "board" && targetType === "header" && draggedCard.bivalenceSell) {
  console.log(`📢 Bivalence de ${draggedCard.nom} désactivée`);
  draggedCard.bivalenceSell(boardPlayer)

}
if (sourceType === "deck" && targetType === "board-drop"  && boardPlayer.length <7){
  let bivalencePresent = boardPlayer.findIndex(carte => carte.bivalence)
  console.log(bivalencePresent)
  if (bivalencePresent >= 0){
      let carteBivalence = boardPlayer.find(carte => carte.bivalence)
