...
export function piocherCarteSpe(sourceType, targetType, draggedCard, deck, setDeck, boardPlayer, setBoardPlayer) {
    if (sourceType === "deck" && targetType === "board-drop" && draggedCard.piocherCarteSpe === true && boardPlayer.length < 7) {
      ...
      // on NE fait PAS le setDeck ici directement
      const nouveauDeck = deck
        .filter((c) => c.id !== draggedCard.id) // on retire la carte posée
        .concat(clone); // on ajoute la nouvelle
  
      // 👇 on renvoie le nouveauDeck pour que le handleDragEnd l'utilise
      return nouveauDeck;
    }
