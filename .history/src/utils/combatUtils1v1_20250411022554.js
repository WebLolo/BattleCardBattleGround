import { coutLvlTaverne, coutLvlTavernePlayer2, getCartesAleatoires, getCartesPourShop, getNombreCartesShop, clonerCarte, infligerDegats } from "@/utils/shopUtils1v1";
...
export async function deroulerCombat1v1({
  ...
  while (cartesPlayer.length > 0 && cartesPlayer2.length > 0) {
      ...
      if(attaquant.furieUse){
        ...
        const indexA = cartesPlayer.findIndex(c => c.id === attaquant.id);
        const indexD = cartesPlayer2.findIndex(c => c.id === defenseur.id);

        if (indexA >= 0) cartesPlayer[indexA] = infligerDegats(attaquant, defenseur.atk);
        if (indexD >= 0) cartesPlayer2[indexD] = infligerDegats(defenseur, attaquant.atk);

        // ➕ Mettre à jour les boards
        setBoardCombat([...cartesPlayer]);
        setBoardCombatPlayer2([...cartesPlayer2]);

        await sleep(800); // le temps de l'anim + popup

        // 🔄 Nettoyer les effets visuels
        if (indexA >= 0) cartesPlayer[indexA].degatsRecus = 0;
        if (indexD >= 0) cartesPlayer2[indexD].degatsRecus = 0;
