...
export async function aoeCible(cartesPlayer, cartesPlayer2) {
  ...
  export async function oneTicDebutCombat(cartesSource, cartesCible, setProjectileAnim) {
    ...
    if (!lanceur) {
        console.warn("Aucune carte avec oneTicDebutCombat trouvée dans cartesSource :", cartesSource);
        return {
          cartesCible,
          morts: []
        };
    ...
    if (sourceEl && targetEl && !hasFiredProjectile) {
        hasFiredProjectile = true;
      }
    const duration = 600; // ⏱️ Durée fixe, identique à celle de ProjectileImage
    if (sourceEl && targetEl) {
        const sRect = sourceEl.getBoundingClientRect();
        const tRect = targetEl.getBoundingClientRect();
        console.log("🔥 setProjectileAnim déclenché depuis", lanceur.nom, "vers", cible.nom);
        setProjectileAnim({
            id: `${lanceur.id}-${cible.id}-${Date.now()}`, // 👈 encore plus unique
        ...
        // ⏳ Attendre l’arrivée du projectile
        await sleep(duration);
        hasFiredProjectile = false;
        console.log("💥 Projectile lancé de", lanceur.nom, "vers", cible.nom);
...
export function degatsAdj(attaquant, defenseur, cartesPlayer, cartesPlayer2, nomDefenseur, setGriffeEffects) {
    ...
    const ajouterEffet = (carte) => {
      ...
      carte.hp -= attaquant.atk;
      carte.degatsRecus = attaquant.atk;
    };
