...
export function piocherCarteInf( sourceType, targetType, draggedCard, deck, setDeck, boardPlayer ){
    if (sourceType === "deck" && targetType === "board-drop" && draggedCard.piocherCarteInf === true && boardPlayer.length <7) {
        const nouveauDeck = deck.filter((c) => c.id !== draggedCard.id);
        const cartesCrocNoir = cards.filter((c) => c.famille === "Croc-Noir");
        console.log("🔍 draggedCard.lvl =", draggedCard.lvl, typeof draggedCard.lvl);
        console.log("🔍 Aperçu cartesCrocNoir.map(lvl) =", cartesCrocNoir.map(c => c.lvl));
        const cartesCrocNoirInf = cartesCrocNoir.filter((c) => {
            console.log(`Comparaison: ${c.nom} (lvl ${c.lvl}) < draggedCard (lvl ${draggedCard.lvl}) =>`, c.lvl < draggedCard.lvl);
            return c.lvl < draggedCard.lvl;
          });
