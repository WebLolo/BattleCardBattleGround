...
export async function aoe(cartesPlayer, cartesPlayer2) {
  ...
  export async function oneTicDebutCombat(cartesPlayer, cartesPlayer2) {
    const cible = cartesPlayer2[Math.floor(Math.random() * defenseurProvoc.length)]
    cartesPlayer.forEach((card) => {
        if(card.oneTicDebutCombat){
            card.oneTicDebutCombat(cible)
        }
    })
