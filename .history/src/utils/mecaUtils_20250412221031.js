...
export async function oneTicDebutCombat(cartesSource, cartesCible, setProjectileAnim) {
    const resultCibles = [...cartesCible]; // On copie pour modification progressive
    const morts = [];
  
    const lanceurs = cartesSource.filter(carte => carte.oneTicDebutCombat);
  
    for (const lanceur of lanceurs) {
      // ⚠️ On choisit une cible parmi les vivants
      ...
      // 💥 Appliquer l'effet
      lanceur.oneTicDebutCombat(cible, lanceur);
  
      // ✅ Mettre à jour les PV de la cible dans le tableau
      const index = resultCibles.findIndex(c => c.id === cible.id);
      if (index !== -1) {
        resultCibles[index] = { ...cible }; // on copie pour éviter les effets de bord
        if (resultCibles[index].hp <= 0) {
          morts.push(resultCibles[index]);
        }
      }
  
      await sleep(200); // petite pause entre les tirs, optionnelle
    }
  
    return { cartesCible: resultCibles.filter(c => c.hp > 0), morts };
  }
