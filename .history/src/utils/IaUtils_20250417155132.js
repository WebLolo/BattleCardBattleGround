  ...
  if (tourIa === 3) {
    ...
    const planC = shopCards.sort((a, b) => b.atk - a.atk)[0];
      if (crocs.length > 0) {
        ...
        if(cible.sousFamille === cibleSousType){
          console.log("Parfait on reste sur le plan A");
          console.log(cible)
        }else{
          console.log("🛑 Aucun Croc-Noir", cibleSousType,  "dans le shop. IA passe au plan B")
          const meilleuresCartesPlanB = shopCards
          .filter((c) => c.famille === "Croc-Noir")
          .sort((a, b) => b.atk - a.atk);
          const cible =
          meilleuresCartesPlanB[0] || shopCards.sort((a, b) => b.atk - a.atk)[0];
          console.log(cible)
        }
