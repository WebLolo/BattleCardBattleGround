...
import { aoe, boardPositionFight } from "@/utils/mecaUtils";
...
export async function deroulerCombat1v1({
  ...
  while (cartesPlayer.length > 0 && cartesPlayer2.length > 0) {
    ...
    if(attaquant.nom === "Zog'Bar le Vent d'Acier" && nomDefenseur === "joueur2" && cartesPlayer2.length > 1){
      ...
      }else if(defenseur === cartesPlayer2[cartesPlayer2.length]){
        cartesPlayer2[cartesPlayer2.length - 1].hp -= attaquant.atk
      }else{
        cartesPlayer2[defenseur.boardPosition - 1].hp -= attaquant.atk
        cartesPlayer2[defenseur.boardPosition + 1].hp -= attaquant.atk
      }
    }
