...
export const acheterCarteIA = ({ cards, goldIA, setGoldIA, boardIA, setBoardIA }) => {
  if (goldIA < 3 || !cards.length || boardIA.length >= 7) return;

  let carteAcheteIA = cards[0];
  if (!carteAcheteIA) return;

  setGoldIA(prev => prev - 3);

  setBoardIA(prevBoard => [
    ...prevBoard,
    clonerCarte({ carte: carteAcheteIA, camp: "ia" })
    
  ]);
  useEffect(() => {
    console.log("✅ Nouveau board IA :", boardIA);
  }, [boardIA]);
};
