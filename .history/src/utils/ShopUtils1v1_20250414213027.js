...
export function fusionnerCartesIdentiques({ carteBase, deck, board, setDeck, setBoard }) {
  // 🔁 On ignore les cartes déjà dorées
  
  const toutesCartes = [...deck, ...board];
  const copies = toutesCartes.filter(c => c.nom === carteBase.nom && !c.estDoree);
  console.log("🔎 Fusion triggered for:", carteBase.nom);
    console.log("📦 Copies trouvées :", copies.map(c => `${c.nom} (${c.id})`));
    console.log("🎯 Retrait ciblé sur nom :", carteBase.nom);
  ...
  if (copies.length >= 3) {
    ...
    for (const carte of deck) {
      if (carte.nom === carteBase.nom && !carte.estDoree && àRetirer > 0) {
        àRetirer--;
      } else {
    ...
    for (const carte of board) {
      if (carte.nom === carteBase.nom && !carte.estDoree && àRetirer > 0) {
        console.log(`❌ Retrait board: ${carte.nom} (${carte.id})`);
        àRetirer--;
      } else {
