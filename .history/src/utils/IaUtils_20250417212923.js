...
export async function TourIa({
  ...
  if (tourIa === 3) {
    ...
    if (crocs.length === 0) {
      console.log("🛑 Aucun Croc-Noir sur le board. IA passe au plan C");
    }
    let gold = goldPlayer2
    const meilleuresCartes = shopCards
    .filter((c) => c.sousFamille === cibleSousType)
    .sort((a, b) => b.atk - a.atk);
    const planB = shopCards
    .filter((c) => c.famille === "Croc-Noir")
    .sort((a, b) => b.atk - a.atk);
    const planC = shopCards.sort((a, b) => b.atk - a.atk)[0];
      if (crocs.length > 0) {
        const cible = meilleuresCartes[0]
        const cibleB = planB[0]
        if(meilleuresCartes.length > 0){
          console.log("Parfait on reste sur le plan A");
          console.log(cible)
        }else{
          console.log("🛑 Aucun Croc-Noir", cibleSousType,  "dans le shop. IA tente de raffraichir le shop")
          let carteTrouveFlag = false
          // while(gold > 3 || carteTrouveFlag === "false" ){
          ...
          const meilleuresCartes1 = shopCards
          .filter((c) => c.sousFamille === cibleSousType)
          .sort((a, b) => b.atk - a.atk);
          console.log(meilleuresCartes1)
        }
